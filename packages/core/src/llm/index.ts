...
function normalizeProfileName(raw: string | undefined): LlmProfileName {
  if (raw === undefined || raw === '100-local') return '100-local'
...
export async function resolveLlmProfile(
  ...
  if (extraction === null) {
    console.warn(`[memoria:llm] profil « ${profile} » : aucun provider d'extraction disponible — couches LLM d'extraction désactivées`)
  }
  if (embeddings === null) {
    console.warn(`[memoria:llm] profil « ${profile} » : embeddings indisponibles — recherche sémantique désactivée (FTS seul)`)
  }
